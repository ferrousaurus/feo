...
describe("sourceId", () => {
  it("returns a path-prefixed id for a local source", () => {
    expect(sourceId({ path: "foo" })).toBe("path:foo");
  });

  it("returns a url-prefixed id for a remote source", () => {
    expect(sourceId({ url: "https://x/y" })).toBe("url:https://x/y");
  });
...
describe("sourceLabel", () => {
  it("returns the raw path for a local source", () => {
    expect(sourceLabel({ path: "foo" })).toBe("foo");
  });

  it("returns the raw url for a remote source", () => {
    expect(sourceLabel({ url: "https://x/y" })).toBe("https://x/y");
  });
